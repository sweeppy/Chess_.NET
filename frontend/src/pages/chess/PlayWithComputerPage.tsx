...
import { OnStartGame } from '../../services/game/computer';
...
const PlayWithComputerPage = () => {
    ...
    const handlePawnMove = (
        ...
        console.log(
            `pawn on ${pawn.position} have moveDistance: ${moveDistance}`
        );
    ...
    const getCastlingRookPositions = (
        ...
        return isKingCastle
            ...
            : {
                  oldPos: `A${king.position[1]}`,
                  newPos: `D${king.position[1]}`,
              };
    };
        ...
        updatedPieces = updatedPieces.map((p) => {
            if (p.position === selectedPiece?.position) {
                ...
                if (p.type === 'king') {
                    ...
                    const offset =
                        letters.indexOf(newPosition[0]) -
                        letters.indexOf(p.position[0]);
                    if (Math.abs(offset) > 1) {
                        castlingRookPositions = getCastlingRookPositions(
                            p,
                            offset
                        );
                    }
                                        ...
                                        {legalMoves?.includes(position) && (
                                            ...
                                            >
                                                <img
                                                    className="img-lm"
                                                    src={
                                                        '/src/assets/game/moves/legal_move.svg'
                                                    }
                                                    alt={'Legal move'}
                                                />
                                            </div>
